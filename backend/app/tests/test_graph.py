...
class TestGraphNode:
    def test_hash_equality_by_type_and_id(self):
        ...
        now = datetime.now(timezone.utc)
        n1 = GraphNode(entity_type="judge", entity_id=1, canonical_entity_id=None,
                       display_name="Alice", confidence=0.9)
        n2 = GraphNode(entity_type="judge", entity_id=1, canonical_entity_id=None,
                       display_name="Alice!", confidence=0.5)
        assert n1 == n2
    ...
    def test_different_ids_not_equal(self):
        ...
        n1 = GraphNode(entity_type="judge", entity_id=1, canonical_entity_id=None,
                       display_name="Alice", confidence=0.9)
        n2 = GraphNode(entity_type="judge", entity_id=2, canonical_entity_id=None,
                       display_name="Alice", confidence=0.9)
        assert n1 != n2
...
class TestGraphPath:
    def _make_path(self, n_nodes=2, n_edges=1):
        ...
        now = datetime.now(timezone.utc)
        nodes = [
            GraphNode(entity_type="judge", entity_id=i, canonical_entity_id=None,
                      display_name=f"n{i}", confidence=0.9)
            for i in range(n_nodes)
        ]
        edges = [
            EdgeRecord(id=i, subject_type="judge", subject_id=i,
                       predicate="presided_over", object_type="case",
                       object_id=i + 10, evidence_refs={},
...
class TestTemporalEdge:
    def test_is_open_no_until(self):
        ...
        from app.graph.edge_models import EdgeRecord
        now = datetime.now(timezone.utc)
        rec = EdgeRecord(id=1, subject_type="j", subject_id=1,
                         predicate="p", object_type="c", object_id=2,
                         evidence_refs={}, valid_from=now, valid_until=None,
                         status="active", created_by="test")
    ...
    def test_is_active_at(self):
        ...
        rec = EdgeRecord(id=1, subject_type="j", subject_id=1,
                         predicate="p", object_type="c", object_id=2,
                         evidence_refs={}, valid_from=now - timedelta(days=5),
                         valid_until=None, status="active", created_by="test")
        te = TemporalEdge(edge=rec, valid_from=now - timedelta(days=5), valid_until=None)
